...
function Overview({onNavigate,show}:{onNavigate:(s:string)=>void;show:(s:string)=>void}) {
  return <div className="content">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel quick"><div className="panel-title"><h3>Quick actions</h3><span>Most used tools</span></div><div className="quick-grid"><button onClick={()=>show('Resource composer opened')}><span className="peach"><Plus/></span><strong>Assign resource</strong><small>Article, link, or PDF</small></button><button onClick={()=>show('Worksheet generator ready')}><span className="lilac"><WandSparkles/></span><strong>Generate worksheet</strong><small>AI-assisted prompts</small></button><button onClick={()=>show('Video assignment created')}><span className="mint"><Video/></span><strong>Assign video</strong><small>Share with the club</small></button><button onClick={()=>onNavigate('Members')}><span className="sand"><Users/></span><strong>Add members</strong><small>Import a signup list</small></button></div></section>
