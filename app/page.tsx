...
function SectionPage({title,show}:{title:string;show:(s:string)=>void}) {
  ...
  const descriptions:Record<string,string>={Resources:'Curate readings, links, and reference material for the club.',Worksheets:'Create discussion-ready worksheets and reflection questions.',Videos:'Assign talks, lectures, and documentaries to members.'};
  return <div className="content"><section className="library-head"><div><p className="eyebrow">CLUB LIBRARY</p><h2>{title}</h2><p>{descriptions[title]}</p></div><button className="primary" onClick={()=>show(`${title.slice(0,-1)} composer opened`)}><Plus size={17}/> Add {title.slice(0,-1).toLowerCase()}</button></section><section className="panel empty-state"><span><Upload size={28}/></span><h3>Your {title.toLowerCase()} workspace is ready</h3><p>Add your first item and publish it when it is ready for students.</p></section></div>
}
